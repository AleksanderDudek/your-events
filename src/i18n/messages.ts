...
const pl = {
  ...
  FILTER_DATE_FROM: 'Od',
  FILTER_DATE_TO: 'Do',
  FILTER_WEEKDAYS: 'Dni tygodnia',
  // Indexed by JS Date#getDay(), so Sunday leads even though the picker shows it
  // last — the index is the day number, not the display position.
  WEEKDAY_SHORT: ['Nd', 'Pn', 'Wt', 'Śr', 'Cz', 'Pt', 'So'],
  WEEKDAY_LONG: [
    'Niedziela',
    'Poniedziałek',
    'Wtorek',
  ...
  SORT_VENUE: 'Miejsce',
  SORT_PRICE: 'Cena',
  SORT_DIR_ASC: 'Sortuj rosnąco',
  SORT_DIR_DESC: 'Sortuj malejąco',
  // The mix is a sample, so its count is the sample's size, not the database's
  // — saying "Znaleziono 36" next to 748 would read as a bug. Mirrors
  // MAP_SHOWN_OF_TOTAL, which says the same thing about pins.
  SORT_MIX_SUMMARY: (shown: number, total: number) => `Miks: ${shown} z ${total} wydarzeń`,
...
const en: Messages = {
  ...
  FILTER_DATE_TO: 'To',
  FILTER_WEEKDAYS: 'Days of the week',
  ...
  SORT_MIX_SUMMARY: (shown: number, total: number) => `Mix: ${shown} of ${total} events`,
